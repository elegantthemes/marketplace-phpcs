...
class ValidatedSanitizedInputSniff extends Sniff {
	 ...
	 * @var string|string[]
	 */
	public $customSanitizingFunctions = [];
	 ...
	 * @var string|string[]
	 */
	public $customUnslashingSanitizingFunctions = [];
	...
	public function process_token( $stackPtr ) {
		...
		if ( \T_DOUBLE_QUOTED_STRING === $this->tokens[ $stackPtr ]['code'] || \T_HEREDOC === $this->tokens[ $stackPtr ]['code'] ) {
			$interpolated_variables = array_map( function ( $symbol ) {
				return '$' . $symbol;
			...
			foreach ( array_intersect( $interpolated_variables, $superglobals ) as $bad_variable ) {
				$this->phpcsFile->addError( 'Detected usage of a non-sanitized, non-validated input variable %s: %s', $stackPtr, 'InputNotValidatedNotSanitized', [
					$bad_variable,
		...
		$error_data = [ $this->tokens[ $stackPtr ]['content'] . '[' . implode( '][', $array_keys ) . ']' ];
		...
		for ( $i = ( $stackPtr + 1 ); $i < $this->phpcsFile->numTokens; $i ++ ) {
			...
			if ( \T_OPEN_SQUARE_BRACKET === $this->tokens[ $i ]['code'] && isset( $this->tokens[ $i ]['bracket_closer'] ) ) {
		...
		if ( false === $validated ) {
			$this->phpcsFile->addError( 'Detected usage of a possibly undefined superglobal array index: %s. Use isset() or empty() to check the index exists before using it', $stackPtr, 'InputNotValidated', $error_data );
		}
		...
		if ( ! $this->is_sanitized( $stackPtr, false ) ) {
	...
	protected function mergeFunctionLists() {
		if ( $this->customSanitizingFunctions !== $this->addedCustomFunctions['sanitize'] ) {
			$this->sanitizingFunctions = $this->merge_custom_array( $this->customSanitizingFunctions, $this->sanitizingFunctions );
		...
		if ( $this->customUnslashingSanitizingFunctions !== $this->addedCustomFunctions['unslashsanitize'] ) {
			$this->unslashingSanitizingFunctions = $this->merge_custom_array( $this->customUnslashingSanitizingFunctions, $this->unslashingSanitizingFunctions );
